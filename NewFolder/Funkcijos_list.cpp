...
void generuotiBalus(vector<double>& nd_rezultatai, double& eg) {
    ...
    for (auto& nd : nd_rezultatai) { // Pakeičiame vektorių į sąrašą
        nd = nd_distribution(gen);
    }
...
void rikiuotiPagalVarda(list<Duomenys>& A) {
    A.sort([](const Duomenys& a, const Duomenys& b) { // Naudojame sąrašo rikiavimo funkciją
...
void skaitytiIsFailo(list<Duomenys>& A, const string& failoPavadinimas) {
    ...
    while (getline(failas, eilute)) {
        ...
        vector<double> visiRezultatai = naujas.nd; // Pakeičiame vektorių į sąrašą
...
void rikiuotiStudentus(list<Duomenys>& geri_studentai, list<Duomenys>& blogi_studentai) {
    ...
    if (rikiavimoPasirinkimas == 1) {
        geri_studentai.sort([](const Duomenys& a, const Duomenys& b) {
            return a.vardas < b.vardas;
        });
        blogi_studentai.sort([](const Duomenys& a, const Duomenys& b) {
            return a.vardas < b.vardas;
        });
